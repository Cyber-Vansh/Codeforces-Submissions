...
 
    seg[pos] = __gcd(seg[pos*2+1],seg[pos*2+2]);
}
 
...
 
void check(int l, int r, int pos, int left, int right, int x, int &bad) {
    if (bad>1) {
        return;
    }
 
    if (left<=l && r<=right && seg[pos]%x==0) {
        return;
    }
 
    if (l==r) {
...
 
    if (left<=mid) {
        check(l,mid,pos*2+1,left,right,x,bad);
    }
    if (right>mid) {
        check(mid+1,r,pos*2+2,left,right,x,bad);
    }
}
 
void solve(){
