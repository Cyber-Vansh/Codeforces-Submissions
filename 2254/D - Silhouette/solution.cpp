...
 
    vector<pair<long long,long long>> items(all(freq));
 
    long long curr = 0;
    long long prev = 0;
    bool check = true;
 
    for(int i=0;i<items.size()-1;i++){
        long long x = items[i+1].first-curr;
 
        if(x%items[i].second!=0 || x/items[i].second<=prev){
            check = false;
            break;
        }
 
        ans[items[i].first] = x/items[i].second;
 
        prev = ans[items[i].first];
        curr += x;
    }
 
    ans[items.back().first] = prev+1;
 
    if(!check){
        cout << -1 << "
";
